        ...
        <div class="card-body">
                ...
                <div class="product-search-result bg-white shadow-soft rounded mt-1 px-3">
                    @foreach($products as $product)
                        <div class="border-bottom py-3 result">
                            <a class="media gap-3" href="{{ route('branch.flash-sale.add-product-to-session', [$flash_sale_id, $product['id']]) }}">
                                <img class="selected-product-img rounded border p-1" width="55" src="{{ $product['image_fullpath'][0] }}" alt="{{ translate('image') }}">
            ...
            <div class="selected-products">
                ...
                @foreach($selected_products as $selected_product)
                    @if($selected_product['id'] == $flash_sale_id)
                        <div class="bg-light rounded selected-product-item p-3 position-relative">
                            <a class="remove-item-btn" href="{{ route('branch.flash-sale.delete-product-from-session', [$flash_sale_id, $selected_product['product_id']]) }}">
                                <i class="tio-clear"></i>
                            </a>
                            <div class="media gap-3">
                                <img class="selected-product-img rounded border p-1" width="55" src="{{ $selected_product['image'] }}" alt="{{ translate('image') }}">
                                <div class="media-body">
            ...
            <div class="d-flex justify-content-end gap-3 mt-3">
                <a class="btn btn-secondary px-5" href="{{ route('branch.flash-sale.delete-all-products-from-session', [$flash_sale_id]) }}">{{ translate('reset') }}</a>
                <a class="btn btn-primary px-5" id="flash-sale-product-store">{{ translate('submit') }}</a>

                <form action="{{ route('branch.flash-sale.add-flash-sale-product', [$flash_sale_id]) }}" method="post" id="product_store" class="d-none">
                    @csrf
                </form>
            </div>
        </div>
                    ...
                    <div class="d-flex flex-wrap justify-content-sm-end gap-2">
                        <form action="{{ url()->current() }}" method="GET">
                            <div class="input-group">
                                <input id="datatableSearch_" type="search" name="search" class="form-control" value="{{ $search }}" placeholder="{{ translate('Search by name') }}" required autocomplete="off">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-2">
                                    <a class="btn btn-outline-danger square-btn form-alert" href="javascript:" data-id="flash-product-delete-{{ $product->id }}" data-message="{{ translate('Want to delete this product ?') }}">
                                        <i class="tio tio-delete"></i>
                                    </a>
                                </div>
                                <form action="{{ route('branch.flash-sale.product.delete', [$flash_sale_id, $product->id]) }}" method="post" id="flash-product-delete-{{ $product->id }}">
                                    @csrf
                                    @method('delete')
                                </form>
                            </td>
