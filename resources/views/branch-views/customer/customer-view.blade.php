    ...
    <div class="d-print-none pb-2">
        <div class="mb-3">
            <h2 class="text-capitalize mb-0 d-flex align-items-center gap-2">
                <img width="20" src="{{asset('assets/admin/img/icons/customer.png')}}"
        ...
        <div class="d-flex flex-wrap justify-content-between gap-3 align-items-center py-3">
            ...
            <a href="{{route('branch.dashboard')}}" class="btn btn-primary">
        ...
        <div class="col-lg-4">
                        ...
                        <div class="media gap-3 flex-wrap align-items-center">
                            ...
                            <div class="media-body">
                                <h4 class="text--primary">
                                    <strong>{{\App\CentralLogics\Helpers::set_symbol($customer?->userAccount?->wallet_balance ?? 0)}}</strong>
                                </h4>
                                <h5><strong>{{translate('Total_Wallet_Balance')}}</h5>
                            </div>
                            <div>
                                <img src="{{ asset('assets/admin/svg/components/wallet.svg') }}" class="img-fit"
                                    alt="wallet image">
                            </div>
                        </div>
                    </div>
            ...
            <div class="card mt-3">
                <div class="card-header">
                    <h4 class="mb-0 d-flex align-items-center gap-2"><i class="tio tio-user"></i>
                        {{translate('customer_information')}}</h4>
                </div>
                @if($customer)
                    <div class="card-body">
                        <div class="media gap-3 flex-wrap align-items-center">
                            <div class="avatar-lg avatar-circle mr-3">
                                <img src="{{$customer['image_fullpath']}}" class="img-fit rounded-circle"
                                    alt="Image Description">
                            </div>
                            <div class="media-body text-dark">
                                <div>{{$customer['f_name'] . ' ' . $customer['l_name']}}</div>
                                <div>
                                    <strong>{{$customer->orders()->where('branch_id', auth('branch')->user()->id)->count()}}</strong>
                    ...
                    <div class="card-body">
                        @forelse($customer?->addresses ?? [] as $key => $address)
                            <ul class="list-unstyled list-unstyled-py-2 text-dark">
                                <li><i class="tio-tab mr-2"></i> {{$address['address_type']}}</li>
                                <li><i class="tio-android-phone-vs mr-2"></i> {{$address['contact_person_number']}}</li>
                                <li>
                                    @if($googleMapStatus && $address['latitude'] && $address['longitude'])
                                        <a class="text-dark" target="_blank"
                                            href="http://maps.google.com/maps?z=12&t=m&q=loc:{{$address['latitude']}}+{{$address['longitude']}}">
                                            <i class="tio-map mr-2"></i> {{$address['address']}}
                                        </a>
                                    @else
                                        <p>{{$address['address']}}</p>
                                    @endif
