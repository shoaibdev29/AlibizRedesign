    ...
    <div class="content container-fluid">
        <div class="mb-3">
            <h2 class="text-capitalize mb-0 d-flex align-items-center gap-2">
                <img width="20" src="{{asset('assets/admin/img/icons/wallet-transaction.png')}}"
            ...
            <div class="card-body">
                <form action="{{request()->fullUrl()}}">
                    <div class="row g-2 mb-4">
                        ...
                        <div class="col-lg-3 col-md-6">
                            <div class="form-group">
                                <label class="input-label">{{ translate('Transaction_Type') }}</label>
                                <select name="transaction_type" id="" class="form-control js-select2-custom mx-1"
                                        title="{{translate('Select_Transaction_Type')}}">
                                    <option value="all" {{ request('transaction_type') == 'all' ? "selected" : "" }}>{{translate('All_Transaction')}}</option>
                                    <option value="credit" {{request('transaction_type') == 'credit' ? "selected" : "" }}>{{translate("credit")}}</option>
                                    <option value="debit" {{request('transaction_type') == 'debit' ? "selected" : "" }}>{{translate("debit")}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-lg-3 col-md-6">
                            <div class="form-group">
                                <label class="input-label">{{ translate('Customer') }}</label>
                    ...
                    <div class="d-flex flex-wrap justify-content-end gap-3">
                        <button type="reset" class="btn btn--secondary max-w-120 flex-grow-1">{{ translate('reset') }}</button>
                        <button type="submit" class="btn btn-primary max-w-120 flex-grow-1">{{ translate('filter') }}</button>
                    </div>
                </form>
            </div>
        </div>
                        ...
                        <div class="d-flex flex-wrap justify-content-sm-end align-items-center gap-3">
                            ...
                            <div>
                                <a class="js-hs-unfold-invoker btn btn-sm btn-white"
                                   href="{{route('branch.report.export-wallet-transaction-history', [ 'start_date'=>request('start_date'), 'end_date'=>request('end_date'), 'transaction_type' => request('transaction_type') ?? 'all', 'customer_id'=>request('customer_id')??'all','search'=>request('search')])}}">
                ...
                <table class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                    ...
                    @foreach($walletTransactions as $key => $walletTransaction)
                        <tr>
                            ...
                            <td class="text-center">{{ ucfirst($walletTransaction->direction) }}</td>
                            <td class="text-right">{{ \App\CentralLogics\Helpers::set_symbol(number_format($walletTransaction->amount, 2)) }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
