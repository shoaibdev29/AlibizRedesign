...
@section('content')
    <div class="content container-fluid">
            ...
            <div class="media gap-3 flex-column flex-sm-row align-items-sm-center">
                ...
                <div class="media-body">
                    <div class="d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-3">
                        ...
                        <a class="btn btn-icon btn-primary rounded-circle" href="{{route('branch.dashboard')}}">
        ...
        <div class="card mb-3">
            <div class="card-body">
                <form action="{{route('branch.report.set-date')}}" method="post">
                    @csrf
                    <div class="row">
                        <div class="col-12">
                            <div class="mb-3">
        ...
        <div class="row">
            @php
                ...
                $to = session('to_date');
                $total=\App\Models\Order::where('branch_id', auth('branch')->user()->id)->whereBetween('created_at', [$from, $to])->count();
                if($total==0){
                   $total=.01;
                }
            @endphp
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $delivered=\App\Models\Order::where('branch_id', auth('branch')->user()->id)->where(['order_status'=>'delivered'])->whereBetween('created_at', [$from, $to])->count()
                @endphp
            ...
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $returned=\App\Models\Order::where('branch_id', auth('branch')->user()->id)->where(['order_status'=>'returned'])->whereBetween('created_at', [$from, $to])->count()
                @endphp
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="row">
            ...
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $failed=\App\Models\Order::where('branch_id', auth('branch')->user()->id)->where(['order_status'=>'failed'])->whereBetween('created_at', [$from, $to])->count()
                @endphp
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="row">
            ...
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $canceled=\App\Models\Order::where('branch_id', auth('branch')->user()->id)->where(['order_status'=>'canceled'])->whereBetween('created_at', [$from, $to])->count()
                @endphp
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="row">
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-3">
                @php
                    $x=1;
                    $y=12;
                    $total=\App\Models\Order::where('branch_id', auth('branch')->user()->id)->whereBetween('created_at', [date('Y-'.$x.'-01'), date('Y-'.$y.'-30')])->count()
                @endphp
                ...
                <div class="hs-unfold">
                    <a class="js-hs-unfold-invoker btn btn-white d-flex gap-1 align-items-center"
                       href="{{route('branch.order.list',['status'=>'all'])}}">
            ...
            @php
                $delivered=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $delivered[$i]=\App\Models\Order::where('branch_id', auth('branch')->user()->id)->where(['order_status'=>'delivered'])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
            ...
            @php
                $ret=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $ret[$i]=\App\Models\Order::where('branch_id', auth('branch')->user()->id)->where(['order_status'=>'returned'])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
            ...
            @php
                $fai=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $fai[$i]=\App\Models\Order::where('branch_id', auth('branch')->user()->id)->where(['order_status'=>'failed'])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
            ...
            @php
                $can=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $can[$i]=\App\Models\Order::where('branch_id', auth('branch')->user()->id)->where(['order_status'=>'canceled'])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
        ...
        <div class="card">
            ...
            <div class="card-body card-body-height">
                @php
                    $orders= \App\Models\Order::where('branch_id', auth('branch')->user()->id)->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])->get();
                @endphp
                <div class="tab-content" id="eventsTabContent">
                    ...
                    @php
                        $orders= \App\Models\Order::where('branch_id', auth('branch')->user()->id)->whereBetween('created_at', [now()->subDays(7)->startOfWeek(), now()->subDays(7)->endOfWeek()])->get();
                    @endphp
