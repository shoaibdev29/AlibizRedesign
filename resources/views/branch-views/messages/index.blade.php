                ...
                <div class="card-body px-2" id="conversation_sidebar">
                    <div class="chat_people media gap-3 mb-4 px-2">
                        <div class="avatar rounded-circle position-relative">
                            <img class="img-fit rounded-circle" src="{{auth('branch')->user()->image_fullpath}}"
                                alt="{{ translate('image') }}">
                            <span class="avatar-status status-sm bg-success"></span>
                        </div>
                        <div class="chat_ib media-body">
                            <h5 class="mb-0">{{auth('branch')->user()->name}}</h5>
                            <span class="fs-12">{{auth('branch')->user()->phone}}</span>
                        </div>
                    </div>
                    ...
                    <div class="customer-list-wrap">
                        ...
                        @if(isset($user))
                            <div class="sidebar_primary_div media gap-3 p-2 mb-2 align-items-center customer-list cursor-pointer view-conversation-message rounded {{$unchecked != 0 ? 'conv-active' : ''}}"
                                data-route="{{route('branch.message.view', [$conv->user_id])}}"
                                data-id="customer-{{$conv->user_id}}" id="customer-{{$conv->user_id}}">
                                <div class="avatar rounded-circle">
                                    <img class="img-fit rounded-circle" src="{{$user['image_fullpath']}}"
                                        alt="{{ translate('image') }}">
                                </div>
                                <h5
...
@push('script_2')
    <script>
        ...
        function viewConvs(url, id_to_active) {
            ...
            // Reset counters and active states
            $('#counter-' + current_selected_user.slice(9)).empty().removeClass("badge badge-info");
            $('#' + current_selected_user).removeClass("conv-active");
        ...
        function renderUserList() {
            $('#loading').show();
            $.ajax({
                url: "{{route('branch.message.get_conversations')}}",
                type: 'GET',
                cache: false,
